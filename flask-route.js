var FlaskRoute = function(routes) {
  if (Object.prototype.toString.call(routes) != "[object Object]") {
    alert('routes must be a dictionary');
  }
  this.routes = routes;
}
...
FlaskRoute.prototype.get_route = function(endpoint, args) {
  ...
  if (!self.routes.hasOwnProperty(endpoint)) {
    alert('Could not find route endpoint: ' + endpoint);
    return false;
  }
  if (!args) {
    /**
    ...
    alert('Could not construct a route for ' + endpoint + ' when no args were passed');
    return false;
  }

  if (Object.prototype.toString.call(args) != "[object Object]") {
    alert('args passed to get_route() must be a dictionary');
    return false;
  }
  if (Object.keys(args).length == 0) {
    alert('empty arguments passed to get_route()')
    return false;
  }
  for (var x in self.routes[endpoint]) {
    var route_args = self.routes[endpoint][x].match(/<(?:\w+\:)?(\w+)>/gi);
      ...
      if (route_args.length == Object.keys(args).length) {
        ...
        for (var i in route_args) {
          ...
          if (!args.hasOwnProperty(arg)) {
            alert(arg + ' wanted but not passed');
            return false;
          }
  ...
  alert('Route could not be constructed');
}
